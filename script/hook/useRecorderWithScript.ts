...
const useScript = (url,theSetter,initConfig) => {
  useEffect(() => {
...
// let CCapture:any;
export function useRecorderWithScript({
  format = 'webm',
  duration = 2,
  framerate = 24,
  fps = 24,
  ...
  const [CCModule, s__CCModule] = useState()
  useScript('@/../script/hook/ccapture.js/src/CCapture.js',s__CCModule,{
    format,
  ...
  const capturer = useMemo(() => {
    // return CCModule
    if (typeof CCModule == "undefined") { return null }
    // @ts-ignore
    return new CCModule({
      format,
      framerate: fps || framerate,
  ...
  }, [format, fps, framerate, motionBlurFrames, showWidget, verbose, CCModule])
